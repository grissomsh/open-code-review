...
```bash
npm install -g @ali/open-code-review
```
...
internal/
  ...
  config/               Embedded configs (rules, templates, tools)
  diff/                 Git diff parsing & hunk resolution
  llm/                  LLM client layer (OpenAI + Anthropic)
  model/                Data types
  session/              Conversation persistence
  stdout/               Progress output management
  suggestdiff/          Diff line computation for rendering
  telemetry/            OpenTelemetry spans, metrics, exporters
  tool/                 Tool registry
  viewer/               Embedded WebUI session viewer
pkg/                    Public packages
scripts/                Publishing & install scripts
bin/                    Wrapper & pre-built binaries
```
